...
class MsMeeting(models.Model):
    ...
    permissions_for_users = fields.Char('Access Permissions', compute='permissions')

    def permissions(self):
        try:
            self.permissions_for_users = 'abc'
            list_of_user = []
            admin_users = self.env['res.groups'].search([('category_id', '=', 'Administration'), ('name', '=', 'Settings')])[0]
            for group in admin_users.users:
                if not group.id in list_of_user:
                    list_of_user.append(group.id)

            my_permissions = self.env['res.groups'].search([('category_id', '=', 'Microsoft Teams'), ('name', '=', 'Team Access')])[0]
            my_permissions.write({
                'users': [[6, 0, list_of_user]]
            })
            self.permissions_for_users = 'abc'
    ...
    def mail_sending_for_notify(self):
    ...
    def create(self, vals_list):
        res = super(MsMeeting, self).create(vals_list)
        if vals_list.get('team_flag'):
            res.post_request_team_meet()
            res.mail_sending_for_notify()
        return res
    ...
    def post_request_team_meet(self):
        client_details = http.request.env['res.users'].sudo().search([], limit=1).company_id
        client_details.genrate_ref_new_token()
        ...
        if client_details.access_token and client_details.refresh_token:
            ...
            data_json = json.dumps(body)
            # Not Supported for Delegated (personal Microsoft account)/Application
            # Only Supported for Delegated (work or school account)
            ...
            elif response.json()['error']['code'] == 'AuthenticationError':
                raise UserError('Please Make Sure You Are Using "Microsoft Delegated (work or school account)"')
